...
void poseCallback(const sensor_msgs::LaserScan::ConstPtr& original_msg){

  std::string source_frame = std::string("laser_link");
  if(!source_frame.compare(original_msg->header.frame_id))
  {
      return;
  }
  ...
  for(int i=0; i<num_points; i++)
  {
    ...
    try{
        transformer.transformPoint("base_link",
              old_g_point, st_g_point);
    }
    catch(tf::TransformException ex)
    {
        ROS_ERROR("Received an exception trying to transform a point from \"laser_link\" to \"base_link\": %s", ex.what());
    }
  ...
  // for(int i=0; i<180; i++){
  //   laser_point.point.x = original_msg->ranges[i]*cos(i);
  //   laser_point.point.y = original_msg->ranges[i]*sin(i);
  ...
  //   listener.transformPoint("axis", laser_point, base_point);
  // }
  // catch(tf::TransformException& ex){
  //   ROS_ERROR("Received an exception trying to transform a point from \"laser_link\" to \"base_link\": %s", ex.what());
  // }
...
int main(int argc, char** argv){
  ...
  ros::Subscriber sub = node.subscribe("/scan", 10, &poseCallback);
  scan_pub = node.advertise<sensor_msgs::LaserScan>("base_scan", 1);
